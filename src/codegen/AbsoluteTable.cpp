...
namespace cg::abs {
	...
	AbsoluteTable::AbsoluteTable(CfgContext const &cfg) {
		_ctx = &cfg;
		_token_size = Token::Type::Eof - Token::Type::Unknown + 1;
	...
	void AbsoluteTable::print(std::ostream &os) {
		...
		for (int i = Token::Type::Unknown; i <= Token::Type::Eof; i++) {
	...
	TableState AbsoluteTable::row_state(StateId const &state_id) {
